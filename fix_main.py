...
# Replace everything from `void telemetry_task_function` up to `* GCS Command Task`
pattern = re.compile(r'void telemetry_task_function\(void \*params\).*?/\* ==+[^=]*?\* GCS Command Task', re.DOTALL)
text = pattern.sub(new_task, text)
